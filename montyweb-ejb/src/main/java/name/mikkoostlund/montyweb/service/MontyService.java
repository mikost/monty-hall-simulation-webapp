...
public class MontyService {
    ...
    private static final Logger log = org.slf4j.LoggerFactory.getLogger(MontyService.class);
    ...
    public SimulationResult runSimulation(int numberOfRuns, int numberOfDoors, Set<ShowContestant> showContestants) {
        log.debug("runSimulation({},{},{})", numberOfRuns, numberOfDoors, showContestants);
        boolean storeResult = sc.isCallerInRole("uzers");
        User user = null;
        if (storeResult) {
            user = em.createQuery("FROM User u WHERE u.name LIKE :userName", User.class).setParameter("userName", sc.getCallerPrincipal().getName()).getSingleResult();
        }

        MontyExperiment montyExperiment = new MontyExperiment(new DefaultShowImpl(new DefaultShowHelper(numberOfDoors)), numberOfRuns, showContestants);
        ...
        SimulationResult simulationResult = new SimulationResult(user, numberOfRuns, numberOfDoors, contestantResults);
        if (storeResult) {
